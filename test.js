...
function run(){
  ...
  ok('no hardcoded Ultimate chip', !qa('#strandChips .chip').some(b=>/^ultimate$/i.test(b.textContent)));
  ...
  if (pullBtn) {
    ...
    ok('setRdr refreshes era counters', html.slice(html.indexOf('function setRdr'), html.indexOf('function setRdr')+900).includes('tallyEras()'));
    ...
    ok('classic skin has character chips', qa('#strandChips .chip').length === D.strands.length);
    q('#tabSettings').click();
