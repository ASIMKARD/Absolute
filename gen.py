...
FRANCHISE = {
    'key':        'absolute',
    'wordmark':   'Absolute',
    'title':      'Absolute Universe \u2014 Reading Order',
    'strapline':  'DC Absolute Universe reading tracker \u00b7 2024\u20132026',
    'theme':      '#0D0616',
    'span':       '2024\u20132026',
    'search_url': 'https://dc.fandom.com/wiki/Special:Search?query=',
    'dual_order': None,
}
# No workbook: this build reads dataset.py (web-app-only spec).
...
from dataset import ERAS, ARCS, ISSUES, CTX, LEGEND, MAINT

# Inline-dataset shim: produces byte-identical `rows` (12-col) and `arows`
# (20-col) to the workbook loader. Everything below this block is the
# untouched franchise-agnostic engine.
ARCI = {a['n']: a for a in ARCS}
rows = []
for n, i in enumerate(ISSUES, 1):
    a = ARCI[i['arc']]
    rows.append([None, n, i['key'], a['era'], i['title'], i['arc'], i['type'],
                 i['mo'], '\u2605' if i['core'] else None, i['flags'],
                 i['note'], None])
arows = []
for n, a in enumerate(ARCS, 1):
    arows.append([n, None, a['era'], a['n'], a['n'], a['year'], None,
                  ', '.join(a['strands']), a['type'], a['mo'], None, None,
...
if any(d[3] == 'Elseworlds (ALT)' for d in rows):
    eras.append('Elseworlds (ALT)')
